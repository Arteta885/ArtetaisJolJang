...
# 방향(Direction)과 유의성(Significance) 추가
...
# CSV로 저장
write.csv(coeff_table, file = "nulcear_kim_effect_results.csv", row.names = FALSE)
...
# Topic factor 변환
coeff_table$Topic <- factor(coeff_table$Topic, levels = 1:5)
...
ggplot(coeff_table, aes(x = Topic, y = Estimate, fill = Effect_Direction)) +
  ...
  scale_fill_manual(values = c("Positive" = "#1b9e77", 
                               "Negative" = "#d95f02", 
                               "Neutral" = "gray70")) +
  labs(title = "Effect of 'kim' on Topic Prevalence (Nulcear/Missile)",
